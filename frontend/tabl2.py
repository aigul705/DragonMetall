...
def display_error_in_historical_table(message):
    ...
    if not table_body:
        print(f"ОШИБКА ФРОНТЕНДА Элемент tbody таблицы 'historical-metals-table' не найден")
        return
...
async def fetch_historical_data_once():
    ...
    try:
        ...
        if all_historical_data_cache:
            ...
            # По умолчанию отобразим данные для первого металла в списке 
            # чтобы при первой загрузке страницы таблица не была пустой, если данные пришли
...
def update_historical_table_on_select(event):
    ...
    if selected_date_raw:
        try:
            ...
            print(f"Ошибка преобразования даты ")
...
print(" Модуль tabl2.py загружен.") 